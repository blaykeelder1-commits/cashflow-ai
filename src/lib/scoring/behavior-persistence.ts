...
export async function savePaymentBehaviorScore(
  ...
  await prisma.client.update({
    where: { id: clientId },
    data: {
      paymentBehaviorScore: Math.round(score.score),
      paymentBehaviorTier: score.tier,
      avgDaysToPay: score.factors.avgDaysToPay,
    },
  });
}
...
export async function getStoredScore(
  ...
  if (!client || client.paymentBehaviorScore === null) {
    return null;
  }
  ...
  return {
    ...
    tier: (client.paymentBehaviorTier as ClientTier) || scoreTierFromScore(client.paymentBehaviorScore),
...
export async function getStoredScoresForOrganization(
  ...
  const clients = await prisma.client.findMany({
    ...
    orderBy: { paymentBehaviorScore: 'desc' },
  });

  return clients
    .filter(c => c.paymentBehaviorScore !== null)
    .map(client => ({
      clientId: client.id,
      clientName: client.name,
      score: client.paymentBehaviorScore!,
...
export async function getScoreStatistics(
  ...
  const aggregate = await prisma.client.aggregate({
    where: {
      organizationId,
      paymentBehaviorScore: { not: null },
    },
    _count: true,
    _avg: { paymentBehaviorScore: true },
    _min: { paymentBehaviorScore: true },
    _max: { paymentBehaviorScore: true },
  });
  ...
  const scores = await prisma.client.findMany({
    ...
    select: { paymentBehaviorScore: true },
    orderBy: { paymentBehaviorScore: 'asc' },
  });

  const medianScore = scores.length > 0
    ? scores[Math.floor(scores.length / 2)].paymentBehaviorScore ?? 0
    : 0;

  return {
    total: aggregate._count,
    avgScore: aggregate._avg.paymentBehaviorScore ?? 0,
    medianScore,
    minScore: aggregate._min.paymentBehaviorScore ?? 0,
    maxScore: aggregate._max.paymentBehaviorScore ?? 0,
  };
}
