...
export async function getChannelEffectiveness(
  ...
  for (const c of channelSuccessCounts) {
    successMap.set(c.actionType, c._count._all);
  }

  const results: ChannelEffectiveness[] = [];

  for (const group of channelCounts) {
    const successfulCount = successMap.get(group.actionType) ?? 0;
    const totalCount = group._count._all;

    // Get best day/hour for this channel
...
export async function updateEffectivenessTable(
  ...
  // Group by tier/industry/actionType
  ...
  for (const action of groups) {
    const key = `${action.client?.paymentBehaviorTier ?? 'null'}|${action.client?.industry ?? 'null'}|${action.actionType}`;
